angular.module("twitter").controller("loginController", [
  "$scope",
  "Twitter",
  "$window",
  function($scope, Twitter, $window) {
    var credentialCheck = function(user, pass) {
    ...
    $scope.login = function() {
      Twitter.login($scope.login.user, $scope.login.pass).then(function(res) {
        if (res.status != 200) {
          alert(
            "Login Failed: Retry with a different username/password combination"
          );
        }
      });
    };
    ...
    $scope.register = function() {
      Twitter.register($scope.register.user, $scope.register.pass).then(
        function(res) {
          if (res.status == 200) {
